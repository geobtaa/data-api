        ...
        try:
            ...
            if table_type == 'spr':
                ...
                if record.get('name') is None or record.get('name') == '':
                    record['name'] = f"Unnamed {record.get('placetype', 'place')} #{record.get('wok_id', 'unknown')}"
                    self.logger.warning(f"Found record with null name, using fallback: {record['name']}")
                ...
                if record.get('ancestor_id') is None or record.get('ancestor_id') == '':
                    self.logger.warning(f"Skipping ancestor record for wok_id {record.get('wok_id')} due to null ancestor_id")
                    return None
                ...
                if record.get('other_id') is None or record.get('other_id') == '':
                    self.logger.warning(f"Skipping concordance record for wok_id {record.get('wok_id')} due to null other_id")
                    return None
                ...
                if record.get('body') is None or record.get('body') == '':
                    self.logger.warning(f"Skipping geojson record for wok_id {record.get('wok_id')} due to null body")
                    return None
                ...
                if record.get('name') is None or record.get('name') == '':
                    self.logger.warning(f"Skipping names record for wok_id {record.get('wok_id')} due to null name")
                    return None
    ...
    async def import_table_data(self, table_type: str) -> Dict[str, Any]:
        ...
        for i, chunk in enumerate(chunks):
            if i > 0 and i % 5 == 0:  # Log progress every 5 chunks
                ...
                self.logger.info(f"{table_type} Progress: {progress:.1f}% - Inserted {inserted:,} of {len(cleaned_records):,} records ({records_per_sec:.1f} records/sec)")
        ...
        self.logger.info(f"Inserted {inserted:,} {table_type} records from {csv_file} in {file_elapsed_time:.2f} seconds ({records_per_second:.1f} records/sec)")
    ...
    async def import_data(self) -> Dict[str, Any]:
        ...
        for table_type in ['spr', 'ancestors', 'concordances', 'geojson', 'names']:
            ...
            if os.path.exists(csv_path):
                ...
                self.logger.warning(f"CSV file not found: {csv_path} - skipping {table_type} table import")
