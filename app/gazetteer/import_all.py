...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('gazetteer_import.log')
    ]
)
...
async def import_all(gazetteer_types: List[str] = None, data_dir: Optional[str] = None) -> Dict[str, Any]:
    ...
    logger.info(f"All imports completed. {total_records} records processed across {len(results)} gazetteers in {elapsed_time:.2f} seconds")
...
def parse_args():
    ...
    parser.add_argument(
        '--data-dir',
        type=str,
        help='Base directory for gazetteer data'
    )
    
    parser.add_argument(
        '--output',
        type=str,
        help='Output file for import results (JSON format)'
    )
