...
from db.models import items, item_allmaps
    ...
    try:
        async with session.get(manifest_url) as response:
            if response.status == 200:
                ...
                logger.warning(f"Failed to fetch manifest {manifest_url} (status: {response.status})")
                return None
    except Exception as e:
...
async def check_allmaps_annotation(session: aiohttp.ClientSession, manifest_url: str) -> Optional[str]:
    ...
    try:
        async with session.get(annotation_url, allow_redirects=True) as response:
            if response.status == 200:
                ...
                logger.warning(f"No Allmaps annotation found for {manifest_url} (status: {response.status})")
                return None
    except Exception as e:
...
async def process_item(item: any, session: AsyncSession, http_session: aiohttp.ClientSession) -> bool:
    ...
    try:
        ...
        await session.execute(
            item_allmaps.delete().where(item_allmaps.c.item_id == item.id)
        )
